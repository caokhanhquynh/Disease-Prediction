...
for index, row in data.iterrows():
    ...
    new_row = {symptom: 0 for symptom in new_symptoms}
    new_row['Disease'] = row['Disease']
    
    for i in range(1, len(row)):
        symptom = row[i]
        if pd.notna(symptom) and symptom.strip() in new_symptoms:
            new_row[symptom.strip()] = 1
    
    new_rows.append(new_row)

new_df = pd.DataFrame(new_rows, columns=['Disease'] + new_symptoms)
